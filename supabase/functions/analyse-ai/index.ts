...
serve(async (req) => {
  ...
  try {
    const { subjectName, testDate, daysUntilTest, noteText, uploadId, userId } = await req.json();
    ...
    const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: `You are a study planning AI. Based on the following student notes for "${subjectName}" with a test in ${daysUntilTest} days (test date: ${testDate}), generate:
